...
void main() {
	...
	if (dot(lightDirA, normalVec) >= DToonTh) {
		...
		LAcontr = vec4(0.0, 0.0, 0.0, 1.0);
    }

    // Toon (Phong)
    vec3 hA = normalize(lightDirA + eyedirVec);
    vec4 SAcontr;
	if (dot(normalVec, hA) >= SToonTh) {
		SAcontr = lightColorA;
	} else {
		SAcontr = vec4(0.0, 0.0, 0.0, 1.0);
	}

    out_color = clamp(ambientLight * ambColor + diffColor * LAcontr + specularColor * SAcontr, 0.0, 1.0); 
