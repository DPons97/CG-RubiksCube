...
void main() {
	...
	float CosAngle = dot(lightDirA, Dir);
	lightColorA = lightColor * clamp((CosAngle - LCosOut) / (LCosIn - LCosOut), 0.0, 1.0);

	// Spherical harmonics ambient
    ambientLight = ((dot(normalVec, ADir) + 1.0)*ambientLightColor)/2.0 + ((1.0 - dot(normalVec, ADir))*ambientLightLowColor)/2.0;
    ...
    // Lambert diffuse
    vec4 LAcontr = diffColor * clamp(dot(lightDirA, normalVec),0.0,1.0) * lightColorA;

    // Phong specular
	vec3 hA = normalize(lightDirA + eyedirVec);
	vec4 SAcontr = pow(clamp(dot(normalVec, hA), 0.0, 1.0), SpecShine) * lightColorA;

    out_color = clamp(SAcontr + ambientLight * ambColor + specularColor * SAcontr, 0.0, 1.0); 
